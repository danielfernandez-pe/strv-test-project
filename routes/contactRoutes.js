import express from 'express';
import * as contactController from '../controllers/contactController.js';
import { verifyToken } from '../middleware/authMiddleware.js';
import { validateRequestFields } from '../middleware/contactValidatorMiddleware.js'

const router = express.Router();

router.post('/contacts', validateRequestFields(['name', 'lastName', 'phone', 'address']), verifyToken, contactController.postContact);
router.put('/contacts/:id', validateRequestFields(['phone', 'address']), verifyToken, contactController.putContact);
router.delete('/contacts/:id', verifyToken, contactController.deleteContact);

export default router;